/******************************************************************************
 * Stepper Speedometer
 * File: stepper_speedometer_v1_06.ino
 ...
 * - Encoder turn adjusts the selected calibration item: RATIO or OFFSET.
 * - Encoder pushbutton toggles selected calibration item: RATIO <-> OFFSET.
 * - Calibration display overrides selected mode and shows MPH plus selected item.
 *
 * Version History:
 * v1.00
 ...
 *
 * v1.06
 * - Changed offset calibration from hold-and-turn to pushbutton-selected mode
 * - Encoder pushbutton toggles calibration target between RATIO and OFFSET
 * - Calibration display now shows the selected target instead of button-held state
 *
...
// In calibration mode, press the encoder pushbutton to toggle whether encoder
// rotation adjusts the ratio or the needle offset.
...
//   CLK/A -> D3
//   DT/B  -> D12
//   SW    -> A1. In calibration mode, press to toggle RATIO/OFFSET adjustment.
//   +     -> 5V
//   GND   -> GND
...
// Calibration adjustment target. Active only while calibration harness is present.
const uint8_t CAL_TARGET_RATIO = 0;
const uint8_t CAL_TARGET_OFFSET = 1;
uint8_t calibrationTarget = CAL_TARGET_RATIO;
...
void handleEncoderCalibration()
{
  ...
  if (encoderA != lastEncoderA) {
    ...
    if (encoderA == HIGH) {
      ...
      if (calibrationTarget == CAL_TARGET_OFFSET) {
        if (encoderB == LOW) {
...
void handleEncoderButton()
{
  ...
  if ((now - encButtonLastChange) > DebounceMs && reading != encButtonStableState) {
    ...
    if (encButtonStableState == LOW) {
      ...
      // Short press release.
      // In calibration mode, toggle which value the encoder adjusts.
      // Outside calibration mode, the encoder button has no daily-use function.
      bool calMode = (digitalRead(calSwitchPin) == LOW);
      if (calMode && !encLongPressHandled) {
        calibrationTarget = (calibrationTarget == CAL_TARGET_RATIO) ? CAL_TARGET_OFFSET : CAL_TARGET_RATIO;

#if DEBUG_SERIAL
        Serial.print(F("Calibration target: "));
        Serial.println(calibrationTarget == CAL_TARGET_RATIO ? F("RATIO") : F("OFFSET"));
#endif
      }

      // Save any pending calibration changes on button release.
      if (ratioDirty) {
        saveRatioToFram();
        ratioDirty = false;
      }
  ...
  // Long press intentionally has no reset function in v1.06.
  // Leave the state flag in place so a long press does not also count as a toggle.
