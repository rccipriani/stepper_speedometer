...
void setEncoder(uint8_t pins) {
  Wire.setInputs((Wire.port&4) | ((pins&2)?1:0) | ((pins&1)?2:0));
  serviceCalibrationInputs();
}

void setHarnessKey(bool level) {
  pinLevels[A1]=level; // legacy standalone debounce tests use this fake pin
  Wire.setInputs((Wire.port&3) | (level?4:0));
}

int main() {
  fresh();
  assert(maxSecondChannelPin==3 && calibrationInterruptPin==12 && calSwitchPin==A0);
  assert(Wire.regs[0]==255 && Wire.regs[1]==255 && Wire.regs[4]==7);
  assert(Wire.regs[8]==0 && Wire.regs[10]==4 && Wire.regs[12]==255);
  pinLevels[A0]=LOW; setEncoder(0);
  unsigned transactions=Wire.transactions;
  serviceCalibrationInputs(); assert(Wire.transactions==transactions);
  // Two changes before servicing: first capture plus current GPIO complete half a cycle.
  Wire.setInputs(5); Wire.setInputs(7); serviceCalibrationInputs();
  assert(encoderQuarterSteps==2);
  setEncoder(1); setEncoder(0); assert(encoderPending==1);
  Wire.shortRead=true; setEncoder(2);
  assert(!expanderReady && !expanderPortValid && encoderPending==0);
  display.labels.clear(); drawScreen(true);
  assert(display.labels.find("CAL I2C")!=std::string::npos);
  Wire.shortRead=false; Wire.present=false;
  clockMs=1000; serviceCalibrationInputs(); assert(!expanderReady);
  addDistance(400); assert(state.odoTenths==1);
  Wire.present=true; Wire.setInputs(0); // held KEY on recovery must not act
  clockMs=1999; serviceCalibrationInputs(); assert(!expanderReady);
  clockMs=2000; serviceCalibrationInputs(); assert(expanderReady && expanderPortValid);
  handleControls(3035); assert(calibrationTarget==CAL_RATIO);
  setHarnessKey(HIGH); handleControls(3100); handleControls(3135);
  assert(calibrationTarget==CAL_RATIO);
  setHarnessKey(LOW); handleControls(3200); handleControls(3235);
  setHarnessKey(HIGH); handleControls(3300); handleControls(3335);
  assert(calibrationTarget==CAL_OFFSET);
  std::cout << "PASS MCP23017: configuration, idle, capture/current, short read, missing device, recovery, held key\n";
  ...
  fresh(); pinLevels[A0]=LOW; setEncoder(0);
  ...
  setHarnessKey(LOW); assert(pollButton(button,A1,0)==0);
  assert(pollButton(button,A1,35)==0);
  setHarnessKey(HIGH); assert(pollButton(button,A1,100)==0);
  assert(pollButton(button,A1,135)==1);
  setHarnessKey(LOW); pollButton(button,A1,200); pollButton(button,A1,235);
  assert(pollButton(button,A1,1235)==2);
  assert(pollButton(button,A1,1300)==0);
  setHarnessKey(HIGH); pollButton(button,A1,1400);
  ...
  for (int target=1;target<=6;++target) {
    setHarnessKey(LOW); handleControls(t); handleControls(t+35);
    setHarnessKey(HIGH); handleControls(t+100); handleControls(t+135);
    assert(calibrationTarget==target%6);
    t+=200;
  }
  calibrationTarget=CAL_DIGITS;
  setHarnessKey(LOW); handleControls(t); handleControls(t+35); handleControls(t+1035);
  assert(layoutInfo && calibrationTarget==CAL_DIGITS);
  setHarnessKey(HIGH); handleControls(t+1100); handleControls(t+1135);
  assert(layoutInfo && calibrationTarget==CAL_DIGITS);
