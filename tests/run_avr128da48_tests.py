"""Run the AVR DA application with fake GPIO/SPI/I2C and the real Switec driver.

python tests/run_avr128da48_tests.py --compiler PATH_TO_ZIG_OR_GXX --switec-source PATH
"""
import argparse
...
def main():
    ...
    schemas = (ROOT/'firmware/avr128da48/StorageLayout.h').read_text()
    ...
    distance = (ROOT/'firmware/avr128da48/Distance.h').read_text()
    ...
    fake += 'void analogReference(uint8_t) {}\nvoid analogReadResolution(uint8_t) {}\n'
    for port, base, count in [('A',0,8),('B',8,6),('C',14,8),('D',22,8),('E',30,4),('F',34,7)]:
        for n in range(count):
            fake += f'#define PIN_P{port}{n} {base+n}\n'
    with tempfile.TemporaryDirectory(prefix='avrda-tests-') as directory:
        build=Path(directory)
        for f in (ROOT/'firmware/avr128da48').iterdir():
        ...
        shutil.copyfile(ROOT/'tests/avr128da48_logic.cpp',build/'test.cpp')
        command=[args.compiler]
        if Path(args.compiler).stem=='zig': command+=['c++']
        executable=build/'test.exe'
        command+=['-std=c++17','-D__AVR_AVR128DA48__','-I',str(build),str(build/'test.cpp'),'-o',str(executable)]
